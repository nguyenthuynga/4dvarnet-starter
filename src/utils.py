...
def load_data_with_moreinputs_data2025(path1, path2, obs_var='SPM'):#I wrote based on Quentin code, ds contains both the values and the dims as Quentin code
    # Load the ground truth data
    gt = xr.open_dataset(path1)
    var = 'SPM' 
    gt = gt.rename({var: 'GT'})
    ...
    # # Use 'broadcast_like' to replicate data along the 'time' dimension to match 'GT'
    bathymetry = gt['bathymetry'].broadcast_like(gt['GT'])

    elev=gt['elev']
    Hs=gt['Hs']
    bed_shear_stress=gt['bed_shear_stress']
    depthAverageVelX=gt['depthAverageVelX']
    depthAverageVelY=gt['depthAverageVelY']
    ...
    ds =  (
	xr.Dataset(dict(
            input=inp, tgt=(gt['GT'].dims, gt['GT'].values), bathymetry=(bathymetry.dims, bathymetry.values),
            elev=(elev.dims, elev.values),
            Hs=(Hs.dims, Hs.values),
            bed_shear_stress=(bed_shear_stress.dims, bed_shear_stress.values),
            depthAverageVelX=(depthAverageVelX.dims, depthAverageVelX.values),
            depthAverageVelY=(depthAverageVelY.dims, depthAverageVelY.values),
        ), inp.coords).load()
        .transpose('time', 'lat', 'lon')
    )

    print('ds in load_data_with_moreinputs_data2025: ', ds)

    return ds.to_array()

def load_data_with_moreinputs_data2025_OSE_with_SCHISM_forcing(path1, path2,path_SCHISM_forcing, obs_var='SPM'):#I wrote based on Quentin code, ds contains both the values and the dims as Quentin code
    # Load the ground truth data
    gt = xr.open_dataset(path1)
    # print('gt: ', gt)
    
    data_SCHISM_forcing=xr.open_dataset(path_SCHISM_forcing)
    
    data_SCHISM_forcing = data_SCHISM_forcing.assign_coords(
    lat=data_SCHISM_forcing.lat.astype("float32"),
    lon=data_SCHISM_forcing.lon.astype("float32"),
    )
    
    # # put forcing on EXACT GT grid (avoid float32/float64 + tiny mismatch issues)
    data_SCHISM_forcing = data_SCHISM_forcing.interp(lat=gt.lat, lon=gt.lon, time=gt.time, method="nearest")
    
    #(optional) convert all float variables to float32 to same format with gt
    # data_SCHISM_forcing = data_SCHISM_forcing.astype({
    # v: "float32"
    # for v in data_SCHISM_forcing.data_vars
    # if data_SCHISM_forcing[v].dtype.kind == "f"
    # })
    
    # print('data_SCHISM_forcing: ', data_SCHISM_forcing)
    var = 'SPM' 
    gt = gt.rename({var: 'GT'})

    # # Use 'broadcast_like' to replicate data along the 'time' dimension to match 'GT'
    bathymetry = data_SCHISM_forcing['bathymetry'].broadcast_like(gt['GT'])

    elev=data_SCHISM_forcing['elev']
    Hs=data_SCHISM_forcing['Hs']
    bed_shear_stress=data_SCHISM_forcing['bed_shear_stress']
    depthAverageVelX=data_SCHISM_forcing['depthAverageVelX']
    depthAverageVelY=data_SCHISM_forcing['depthAverageVelY']
