...
#_AugMoreRemovedPatches
#Add baythmetry and Mud fraction
#still daily (and not hourly) reconstruction
#change TrainingItem to add bathymetry and mud fraction
#change load_data_with_bathymetry_and_mud_fraction in utils
# change post_fn and post_fn_rand to have add bathymetry and mudfraction
#change XrDataset because data.dims should be lon lat time which is of the input and tgt, not bathymetry and mud fraction (lon and lat only), da_dims = dict(zip(self.da['input'].dims, self.da['input'].shape)): get back to old one
#replace post_fn_rand = self.post_fn()  # self.post_fn_rand() to check bug related to input keyworkds: get back to old one
#change in load_data_with_bathymetry_and_mud_fraction to have bathymetry and mud fraction same size with spm  by replace them with spm first!!!!
TrainingItem = namedtuple(
    "TrainingItem", ["input", "tgt", "baythmetry", "mudfraction"]
) 
...
class XrDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        #modify below line:
        # da_dims = dict(zip(self.da['input'].dims, self.da['input'].shape)) #da is either training/val/test set# I am currently fixxing the bug here , check conversation on chatgpt
        da_dims = dict(zip(self.da.dims, self.da.shape))#the old one
...
class BaseDataModule(pl.LightningDataModule):#change post_fn
    ...
    def train_mean_std(self, variable='tgt'):#to adress that we have more variables (bathmetry and mud fraction)
        train_data = self.input_da.sel(self.xrds_kw.get(
            'domain_limits', {})).sel(self.domains['train'])
        # print(f"Accessing train_data: {train_data}")

        # print('we are passing here 1')
        
        
        # Check if train_data is a Dataset or a DataArray
        if isinstance(train_data, xr.Dataset):
            # Direct access if it's a Dataset
            # print('train_data is a dataset')
            variable_data = train_data[variable]
        elif isinstance(train_data, xr.DataArray):
            # print('train_data is a dataarray')
            # If it's a DataArray, use the 'variable' coordinate for selection
            variable_data = train_data.sel(variable=variable)
        else:
            raise TypeError("train_data must be xarray.DataArray or xarray.Dataset")
    
        # print('we are passing here 2')
        print('Computing norm_stats for variable: ', variable, variable_data.mean().values.item(), variable_data.std().values.item())

        return variable_data.mean().values.item(), variable_data.std().values.item()


    def post_fn(self):

        # Normalization function for the target (SPM) and input
        normalize_spm = lambda item: (item - self.norm_stats()[0]) / self.norm_stats()[1]

        # Assuming similar statistics can be used for baythmetry and mudfraction or define new ones
        # For example, if different normalization is needed, calculate as:
        m_bath, s_bath = self.train_mean_std(variable='baythmetry')

        m_mud, s_mud = self.train_mean_std(variable='mudfraction')

        normalize_baythmetry = lambda item: (item - m_bath) / s_bath
        normalize_mudfraction = lambda item: (item - m_mud) / s_mud
        ...
        return ft.partial(
            ...
            [
                TrainingItem._make,
                lambda item: item._replace(tgt=normalize_spm(item.tgt)),
                lambda item: item._replace(input=normalize_spm(item.input)),
                lambda item: item._replace(baythmetry=normalize_baythmetry(item.baythmetry)),
                lambda item: item._replace(mudfraction=normalize_mudfraction(item.mudfraction)),
            ],
        )
    ...
    def post_fn_rand(self):
        ...
        # Assuming similar statistics can be used for baythmetry and mudfraction or define new ones
        # For example, if different normalization is needed, calculate as:
        m_bath, s_bath = self.train_mean_std('baythmetry')
        m_mud, s_mud = self.train_mean_std('mudfraction')
        ...
        return ft.partial(
            ...
            [
                TrainingItem._make,
                lambda item: item._replace(input=normalize_spm(self.rand_obs(item.tgt))),
                lambda item: item._replace(tgt=normalize_spm(item.tgt)),
                lambda item: item._replace(baythmetry=normalize_baythmetry(item.baythmetry)),
                lambda item: item._replace(mudfraction=normalize_mudfraction(item.mudfraction)),
            ],
    ...
    def rand_obs(self, gt_item):#here we replace obs_obj = .5*np.sum(obs_mask_item_t_) by different scenarios
        ...
        for t_ in range(dtime):
            ...
            obs_mask_item_t_ = obs_mask_item[t_].copy()
            if np.sum(obs_mask_item_t_)>.05*dlat*dlon:#if less than 5% we accept it as it is
                if random_number<0.67:
                    ratio=random.uniform(0.05, 0.2)
                else:
                    ratio=random.uniform(0.2, 0.7)
                if random_number2<0.4:#hidden all this time stamp
                    obs_mask_item[t_]=np.zeros_like(obs_mask_item[t_])
                else:
                    obs_obj = ratio*np.sum(obs_mask_item_t_)
                ...
                # print("t_: ",t_, np.sum(obs_mask_item[t_])/obs_mask_item[t_].size*100)
        # print("*******************************")
