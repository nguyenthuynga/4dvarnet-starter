...
class XrDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if check_full_scan:
            for dim in patch_dims:
                if (da_dims[dim] - self.patch_dims[dim]) % self.strides.get(
                    dim, 1
                ) != 0:
    ...
    def reconstruct_from_items(self, items, weight=None):
        ...
        das = [
            xr.DataArray(it.numpy(), dims=dims, coords=co.coords)
...
class AugmentedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.noise_sigma is not None:
            noise = (
                np.random.randn(*item.input.shape).astype(np.float32) * self.noise_sigma
            )

        return item._replace(
            input=noise
            + np.where(
                np.isfinite(perm_item.input), item.tgt, np.full_like(item.tgt, np.nan)
            )
        )
...
class BaseDataModule(pl.LightningDataModule):  #
    def __init__(
        self, input_da, domains, xrds_kw, dl_kw, aug_kw=None, norm_stats=None, **kwargs
    ):
        super().__init__()
        self.input_da = input_da
        self.domains = (
            domains  # from which date to which date, specific for train, val, test
        )
        self.xrds_kw = xrds_kw  # patch_dims: { time: 15, lat: 240, lon: 300}, strides: { time: 1, lat: 240, lon: 300}
        self.dl_kw = dl_kw  # {batch_size: 16, num_workers: 1}
        self.aug_kw = aug_kw if aug_kw is not None else {}  # aug_factor: 1
