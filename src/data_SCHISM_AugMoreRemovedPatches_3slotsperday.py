...
#### post_fn_rand to put every where train/test (val is still with post_fn)
...
class BaseDataModule(pl.LightningDataModule):
    ...
    def rand_obs(self, gt_item):#here we replace obs_obj = .5*np.sum(obs_mask_item_t_) by different scenarios
        ...
        for t_ in range(dtime):
            ...
            if np.sum(obs_mask_item_t_)>.05*dlat*dlon:#if less than 5% we accept it as it is
                ...
                if random_number2<0.8:#hidden all this time stamp, I increase this to match with the fact that in OSE we won't have many time step data
    ...
    def setup(self, stage='test'):
        # Function to select specific hours
        def select_hours(da, hours):
            # Extract hour from datetime64[ns] format
            hour = da['time'].dt.hour
            return da.where(hour.isin(hours), drop=True)

        # Specific hours you want to keep (0, 8, 16 corresponding to 12 AM, 8 AM, 4 PM)
        hours_of_interest = [0, 8, 16]

        # Apply the hour selection to the input data array for each domain
        train_data = select_hours(self.input_da.sel(self.domains['train']), hours_of_interest)
        val_data = select_hours(self.input_da.sel(self.domains['val']), hours_of_interest)
        test_data = select_hours(self.input_da.sel(self.domains['test']), hours_of_interest)
        ...
        self.val_ds = XrDataset(
            val_data, **self.xrds_kw, postpro_fn=post_fn,
        )
        self.test_ds = XrDataset(
            test_data, **self.xrds_kw, postpro_fn=post_fn,
        )
