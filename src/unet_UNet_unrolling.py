...
class UNetModel2(nn.Module):
    ...
    def predict(self, x, timesteps, extra):
        ...
        # after result = self.out(h)
        


        result = self.out(h)
        
        # last = self.out[-1]  # last is the conv inside zero_module
        # print("last conv |w| mean:", last.weight.abs().mean().item(), "grad is None?", last.weight.grad is None)
        
        # print("Inside UNet model 2: x rms:", torch.sqrt((x**2).mean()).item(),
        #         "Inside UNet model 2:  result = self.out(h) rms:", torch.sqrt((result**2).mean()).item())
